...
- 모바일 날짜 탭 고정, 48px 재생 버튼, 가로 스크롤 필터 등 터치 중심 조작
- 모바일 정보 순서: 날짜/팀 선택 → 지도 → 현재 구간 → 날씨 → 요약 → 전체 구간
...
재생 마커는 평상시 숨겨 시작점의 `1 Alba` 번호를 가리지 않으며, 코스 재생 중에만 표시된다.
...
- Day 2 GPX는 A/B팀 산악 코스를 모두 담지만 Sass Pordoi에서 하부역으로 내려오는 역방향 케이블카는 같은 선으로 표현한다. Day 5 GPX는 Scoiattoli에서 Averau까지의 일부 트랙이다. 전체 코스 재생을 GPX가 덮어쓰지 않게 `trackPlayback`은 Day 1과 Day 6에만 켜두었다.
